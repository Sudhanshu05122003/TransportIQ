...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'New Load Alert', message: 'A new shipment has been listed on the Load Board (Mumbai to Delhi). Place a quote now.', time: '10 mins ago', unread: true, type: 'bid' },
    { id: 2, title: 'Driver Assigned', message: 'Driver Rajesh Kumar has successfully accepted the assigned trip #TIQ4KF2M.', time: '2 hours ago', unread: true, type: 'assignment' },
    { id: 3, title: 'Compliance Warning', message: 'Insurance for Vehicle MH 12 AB 3456 is expiring in 7 days.', time: '1 day ago', unread: false, type: 'compliance' },
    { id: 4, title: 'Settlement Cleared', message: 'Weekly settlement for transporter account was processed and credited successfully.', time: '3 days ago', unread: false, type: 'payment' }
  ]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Transporter Notifications</h1>
          <p className="text-gray-500 text-sm mt-1">Updates on load assignments, bids, driver actions, and fleet compliance alerts</p>
        </div>
      ...
      <div className="card divide-y divide-gray-100">
        {notifications.length > 0 ? (
          notifications.map(n => (
            <div key={n.id} className={`p-6 flex gap-4 transition-colors hover:bg-gray-50/50 ${n.unread ? 'bg-indigo-50/30' : ''}`}>
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                n.type === 'bid' ? 'bg-indigo-50 text-indigo-600' :
                n.type === 'assignment' ? 'bg-emerald-50 text-emerald-600' :
                n.type === 'compliance' ? 'bg-red-50 text-red-600' :
          ...
          <div className="p-20 text-center">
            ...
            <p className="text-gray-500 mt-1">You have no new transporter notifications.</p>
          </div>
        )}
      </div>
    </div>
